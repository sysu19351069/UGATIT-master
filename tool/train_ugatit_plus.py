...
from configs.cfgs_ugatit_plus import cfgs
import torch
...
class Train:
    ...
    def init(self):
        ...
        self.crit_gan = GANLoss(opt.gan_mode).cuda()  # .cuda必须加
    ...
    def update_G(self, inp):
        ...
        if lambda_idt > 0:  # identity loss
            ...
            if torch.cuda.is_available():
                z_aa = z_aa.cuda()
                z_bb = z_bb.cuda()
            idt_A, cam_bb, _ = self.model.G_A(real_B, z_bb)  # 用于域渐变插值
            idt_B, cam_aa, _ = self.model.G_B(real_A, z_aa)
        ...
        fake_e_bb = self.model.G_A(fake_B, None, True)
        fake_e_aa = self.model.G_B(fake_A, None, True)
